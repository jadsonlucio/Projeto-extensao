...
class frame_media_exponencial(frame):
    def __init__(self,janela,container):
        frame.__init__(self,janela,"frame_media_exponencial",container)
    ...
    def func_aplicar(self):
        try:
            ...
            for serie in series_selecionadas:
                inst_processamento.operacoes_series.media_movel_exponencial(serie,float(self.lag.get()))
            self.janela.destroy()
        except Exception as e:
            print(str(e))

    def iniciar_componentes(self):
        self.label_lag=tk.Label(self,text="Digite a constante de\n regularização(0-1):")
